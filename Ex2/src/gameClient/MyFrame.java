...
public class MyFrame extends JFrame{
	...
	private void drawGraph(Graphics g) {
		...
		while(iter.hasNext()) {
			...
			drawNode(n,5,g);
			Iterator<edge_data> itr = gg.getE(n.get_key()).iterator();
			while(itr.hasNext()) {
	...
	private void drawNode(node_data n, int r, Graphics g) {
		geo_location pos = n.get_location();
		geo_location fp = this._w2f.world2frame(pos);
		g.fillOval((int)fp.x()-r, (int)fp.y()-r, 2*r, 2*r);
		g.drawString(""+n.get_key(), (int)fp.x(), (int)fp.y()-4*r);
	}
	private void drawEdge(edge_data e, Graphics g) {
		directed_weighted_graph gg = _ar.getGraph();
		geo_location s = gg.getNode(e.get_src()).get_location();
		geo_location d = gg.getNode(e.get_dest()).get_location();
